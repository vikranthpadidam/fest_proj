...
const Cricket = () => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:5000/api/auth/getMatches_crk")
      .then((response) => setMatches(response.data))
      .catch((error) => console.error(error));
    
    axios
      .get(`/api/auth/sportsItems/name/cricket`)
      .then((response) => {
  ...
  const handleAddMatch = () => {
    if (selectedMatch) {
      ...
      axios
        .put(
          `http://localhost:5000/api/auth/updateMatch_crk/${selectedMatch._id}`,
          newMatch
        )
        .then((response) => {
          setMatches(
      ...
      // Add new match
      axios
        .post("http://localhost:5000/api/auth/addMatch_crk", newMatch)
        .then((response) => {
  ...
  const handleDeleteMatch = () => {
    if (selectedMatch) {
      axios
        .delete(
          `http://localhost:5000/api/auth/deleteMatch_crk/${selectedMatch._id}`
        )
        .then(() => {
          setMatches(
  ...
  const handleRowClick = (match) => {
    ...
    axios
      .get(`http://localhost:5000/api/auth/getScores_crk/${match._id}`)
      .then((response) => setScores(response.data))
      .catch((error) => console.error(error));
    axios
      .get(
        `http://localhost:5000/api/auth/getPlayers_crk/${match._id}?team=TeamA`
      )
    ...
    axios
      .get(
        `http://localhost:5000/api/auth/getPlayers_crk/${match._id}?team=TeamB`
      )
  ...
  const handleAddScore = () => {
    if (selectedMatch) {
      axios
        .post(
          `http://localhost:5000/api/auth/addScores_crk/${selectedMatch._id}`,
          {
            ...scoreFormData,
  ...
  const handleUpdateScore = () => {
    if (selectedMatch && selectedScoreDetails && selectedScoreDetails._id) {
      ...
      axios
        .put(
          `http://localhost:5000/api/auth/updateScoredetails_crk/${selectedMatch._id}/${scoreId}`,
          {
            ...scoreFormData,
            round1: parseInt(scoreFormData.round1), // Parse round1 to integer
          }
  ...
  const handleDeleteScore = () => {
    if (selectedScoreDetails) {
      ...
      axios
        .delete(
          `http://localhost:5000/api/auth/deleteScoredetails_crk/${scoreId}/${matchId}`
        )
        .then(() => {
  ...
  const handleAddPlayer = () => {
    if (selectedMatch) {
      axios
        .post(
          `http://localhost:5000/api/auth/addPlayers_crk/${selectedMatch._id}`,
          {
            ...playerFormData,
  ...
  const handleUpdatePlayer = () => {
    if (selectedMatch && selectedPlayerDetails) {
      ...
      axios
        .put(
          `http://localhost:5000/api/auth/updatePlayerDetails_crk/${matchId}/${playerId}`,
          {
  ...
  const handleDeletePlayer = () => {
    if (selectedPlayerDetails) {
      ...
      axios
        .delete(
          `http://localhost:5000/api/auth/deletePlayerDetails_crk/${playerId}/${matchId}`
        )
        .then(() => {
          const updatedPlayers =
