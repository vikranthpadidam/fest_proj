...
const Chess = () => {
  ...
  const handleRowClick = (match) => {
    ...
    axios
      .get(`http://localhost:5000/api/auth/getScores_chess/${match._id}`)
  ...
  const handleAddScore = () => {
    if (selectedMatch) {
      axios
        .post(
          `http://localhost:5000/api/auth/addScores_chess/${selectedMatch._id}`,
          {
            ...scoreFormData,
  ...
  const handleUpdateScore = () => {
    if (selectedMatch && selectedScoreDetails && selectedScoreDetails._id) {
      ...
      axios
        .put(
          `http://localhost:5000/api/auth/updateScoredetails_chess/${selectedMatch._id}/${scoreId}`,
          {
            ...scoreFormData,
            round1: parseInt(scoreFormData.round1), // Parse round1 to integer
          }
  ...
  const handleDeleteScore = () => {
    if (selectedScoreDetails) {
      ...
      axios
        .delete(
          `http://localhost:5000/api/auth/deleteScoredetails_chess/${scoreId}/${matchId}`
        )
        .then(() => {
