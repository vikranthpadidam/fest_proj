...
const Kabbadi = () => {
  ...
  const handleAddMatch = () => {
    if (selectedMatch) {
      ...
      // Add new match
      axios
        .post("http://localhost:5000/api/auth/addMatch_kab", newMatch)
        .then((response) => {
  ...
  const handleRowClick = (match) => {
    ...
    axios
      .get(`http://localhost:5000/api/auth/getScores_kab/${match._id}`)
  ...
  const handleAddScore = () => {
    if (selectedMatch) {
      axios
        .post(
          `http://localhost:5000/api/auth/addScores_kab/${selectedMatch._id}`,
          {
            ...scoreFormData,
  ...
  const handleUpdateScore = () => {
    if (selectedMatch && selectedScoreDetails && selectedScoreDetails._id) {
      ...
      axios
        .put(
          `http://localhost:5000/api/auth/updateScoredetails_kab/${selectedMatch._id}/${scoreId}`,
          {
            ...scoreFormData,
            round1: parseInt(scoreFormData.round1), // Parse round1 to integer
          }
  ...
  const handleDeleteScore = () => {
    if (selectedScoreDetails) {
      ...
      axios
        .delete(
          `http://localhost:5000/api/auth/deleteScoredetails_kab/${scoreId}/${matchId}`
        )
        .then(() => {
