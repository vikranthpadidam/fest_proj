...
const Tabletennis = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`/api/auth/sportsItems/name/tabletennis`)
      .then((response) => {
  ...
  const handleAddMatch = () => {
    if (selectedMatch) {
      ...
      // Add new match
      axios
        .post("http://localhost:5000/api/auth/addMatch_tt", newMatch)
        .then((response) => {
  ...
  const handleRowClick = (match) => {
    ...
    axios
      .get(`http://localhost:5000/api/auth/getScores_tt/${match._id}`)
  ...
  const handleAddScore = () => {
    if (selectedMatch) {
      axios
        .post(
          `http://localhost:5000/api/auth/addScores_tt/${selectedMatch._id}`,
          {
            ...scoreFormData,
...
const handleUpdateScore = () => {
  if (selectedMatch && selectedScoreDetails && selectedScoreDetails._id) {
    ...
    axios
      .put(
        `http://localhost:5000/api/auth/updateScoredetails_tt/${selectedMatch._id}/${scoreId}`,
        {
          ...scoreFormData,
          round1: parseInt(scoreFormData.round1), // Parse round1 to integer
        }
  ...
  const handleDeleteScore = () => {
    if (selectedScoreDetails) {
      ...
      axios
        .delete(
          `http://localhost:5000/api/auth/deleteScoredetails_tt/${scoreId}/${matchId}`
        )
        .then(() => {
