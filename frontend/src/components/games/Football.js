...
const Football = () => {
  ...
  const handleAddMatch = () => {
    if (selectedMatch) {
      ...
      axios
        .put(
          `http://localhost:5000/api/auth/updateMatch/${selectedMatch._id}`,
          newMatch
        )
        .then((response) => {
          setMatches(
  ...
  const handleDeleteMatch = () => {
    if (selectedMatch) {
      axios
        .delete(
          `http://localhost:5000/api/auth/deleteMatch/${selectedMatch._id}`
        )
        .then(() => {
          setMatches(
  ...
  const handleRowClick = (match) => {
    ...
    axios
      .get(`http://localhost:5000/api/auth/getScores/${match._id}`)
      .then((response) => setScores(response.data))
      .catch((error) => console.error(error));
    axios
      .get(`http://localhost:5000/api/auth/getPlayers/${match._id}?team=TeamA`)
      .then((response) => setPlayersTeamA(response.data))
      .catch((error) => console.error(error));

    axios
      .get(`http://localhost:5000/api/auth/getPlayers/${match._id}?team=TeamB`)
